function [X, f_val_X, stats_proj, all_L, objs, opt_lambdas, time, lower_bound] = ...
    FBPG_new(A, X, X_grad_phi, X_val_phi, rhs, f_oracle, max_iter, tol_subp, tol,  ...
    L0, beta, gamma)
...
%               L0: initial estimate of smoothness. Typical value 1000 
...
stats_proj = zeros(2, 0); all_L = []; objs = [f_val_X];
opt_lambdas = []; lower_bound = [best_LB];
...
for k = 1:max_iter
    ...
    f_val_X = f_val_X_cand; best_UB = min(f_val_X, best_UB);  L_old = L;    
    objs(end+1) = f_val_X; all_L(end+1) = L; theta_old = theta;
